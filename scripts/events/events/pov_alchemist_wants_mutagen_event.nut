this.pov_alchemist_wants_mutagen_event <- this.inherit("scripts/events/event", {
	m = {
		Peddler = null,
		Merchant = null,
		Reward = 0,
	...
	function create()
	{
		this.m.ID = "event.pov_alchemist_wants_mutagen";
		...
		this.m.Screens.push({
			ID = "Peddler",
			Text = "[img]gfx/ui/events/event_184.png[/img]{%peddler% saunters up with a grin, slipping between you and the wild-eyed anatomist like a knife through cloth. Without so much as a glance your way, he launches into a tirade of shouts, gestures, and accusations about the quality of the goods and the rarity of the specimen. The anatomist, flustered, snaps back just as fiercely, the two of them tossing numbers and insults into the air with such speed and venom it feels more like a tavern brawl than a barter. After a few breathless moments, %peddler% turns on his heel and strolls back to you. %SPEECH_ON% Got him where we want him. He\'s offering %reward% crowns now. I\'ll leave you to squeeze the rest out of him—I\'m off to find something less loud to spend it on.%SPEECH_OFF% With a casual pat on your shoulder, he vanishes into the crowd, leaving you with a much-improved offer.}",
			Image = "",
			List = [],
			Characters = [],
			Options = [
	...
	function onUpdateScore()
	{
		...
		this.m.Town = town;
		this.m.Score = 5 + candidates_items.len()*3;
	}
