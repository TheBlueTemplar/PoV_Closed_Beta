this.autopsy_schrat_blueprint <- this.inherit("scripts/crafting/blueprint", {
	m = {},
	function create()
	{
		this.blueprint.create();
		this.m.ID = "blueprint.autopsy_schrat";
		this.m.Type = this.Const.Items.ItemType.Usable;
		this.m.PreviewCraftable = this.new("scripts/items/autopsy_schrat_item");
		this.m.Cost = 120;
		local ingredients = [
			{
				Script = "scripts/items/misc/corpse_schrat_item",
				Num = 1
			},
		];
	...
	function onCraft( _stash )
	{
		...
		if (this.Math.rand(1, 100) <= 30)
			{
				_stash.add(this.new("scripts/items/misc/ancient_wood_item"));
			}
		if (this.Math.rand(1, 100) <= 30)
			{
