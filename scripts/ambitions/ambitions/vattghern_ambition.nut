this.vattghern_ambition <- this.inherit("scripts/ambitions/ambition", {
	m = {},
	function create()
	{
		this.ambition.create();
		this.m.ID = "ambition.vattghern";
	...
	function onReward()
	{
		...
		local item = this.new("scripts/items/accessory/vattghern_medallion_item");
	...
	function onPrepareVariables( _vars )
	{
		...
		foreach( bro in brothers )
		{
			if (bro.getSkills().hasSkill("trait.witcher") && bro.getCurrentProperties().getBravery() > highestBravery)
			{
				bestSergeant = bro;
				highestBravery = bro.getCurrentProperties().getBravery();
			}
