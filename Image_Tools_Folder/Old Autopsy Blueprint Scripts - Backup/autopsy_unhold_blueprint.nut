this.autopsy_unhold_blueprint <- this.inherit("scripts/crafting/blueprint", {
	m = {},
	function create()
	{
		this.blueprint.create();
		this.m.ID = "blueprint.autopsy_unhold";
		this.m.Type = this.Const.Items.ItemType.Usable;
		this.m.PreviewCraftable = this.new("scripts/items/autopsy_unhold_item");
		this.m.Cost = 120;
		local ingredients = [
			{
				Script = "scripts/items/misc/corpse_unhold_item",
				Num = 1
			},
		];
	...
	function onCraft( _stash )
	{
		...
		if (this.Math.rand(1, 100) <= 30)
			{
				_stash.add(this.new("scripts/items/loot/unhold_hide_item"));
			}
		if (this.Math.rand(1, 100) <= 30)
			{
